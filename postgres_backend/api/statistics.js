...
statisticsRouter.get("/substats", async (req, res) => {
  try {
    ...
    const artifactsWithSubs = rows.map(row => ({
      ...
      sub_ATK_per: parseInt(row.Percent_ATK, 10),
      sub_HP_per: parseInt(row.Percent_HP, 10),
      sub_DEF_per: parseInt(row.Percent_DEF, 10),
      ...
      sub_Crit_Rate: parseInt(row.Crit_Rate, 10),
      sub_Crit_DMG: parseInt(row.Crit_DMG, 10),
  ...
  try {
    const query = `
      SELECT 
          ...
          SUM(CASE WHEN l."Added_substat" = '%ATK' THEN 1 ELSE 0 END) AS "AddedSubstat_Percent_ATK",
          SUM(CASE WHEN l."Added_substat" = '%DEF' THEN 1 ELSE 0 END) AS "AddedSubstat_Percent_DEF",
          SUM(CASE WHEN l."Added_substat" = '%HP' THEN 1 ELSE 0 END) AS "AddedSubstat_Percent_HP",
          ...
          SUM(CASE WHEN l."Added_substat" = 'Crit Rate' THEN 1 ELSE 0 END) AS "AddedSubstat_Crit_Rate",
          SUM(CASE WHEN l."Added_substat" = 'Crit DMG' THEN 1 ELSE 0 END) AS "AddedSubstat_Crit_DMG",
    ...
    const artifactsWithSubs = rows.map(row => ({
      ...
      roll_HP_per: parseInt(row.L_HP_per, 10),
      roll_ATK_per: parseInt(row.L_ATK_per, 10),
      roll_DEF_per: parseInt(row.L_DEF_per, 10),
      roll_EM: parseInt(row.L_EM, 10),
      roll_ER: parseInt(row.L_ER, 10),
      roll_Crit_Rate: parseInt(row.L_Crit_Rate, 10),
      roll_Crit_DMG: parseInt(row.L_Crit_DMG, 10),
